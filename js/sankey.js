...
class SankeyChart {
  ...
  render(flows, mode = 'sw') {
    ...
    sortedFlows.forEach(flow => {
      ...
      // NEW: Flow percentage label (만약 흐름이 어느 정도 두께가 있다면)
      if (flowH > 10) {
        ctx.globalAlpha = 0.8;
        ...
        const midY = sy + (ty - sy) * 0.5 + flowH / 2;
        const pctSource = Math.round((flow.value / sn.val) * 100);
        ctx.fillText(`${pctSource}%`, midX, midY);
      }
  ...
  static buildSWFlows(documents) {
    ...
    documents.forEach(doc => {
      ...
      if (doc.simulation.status === 'matched' || doc.simulation.status === 'unclassified') {
        target = doc.simulation.newPath?.swName || '미분류';
      }
  ...
  static buildCategoryFlows(documents) {
    ...
    documents.forEach(doc => {
      ...
      let target = '미이관 (필터링됨)';
      if (doc.simulation.status === 'matched' || doc.simulation.status === 'unclassified') {
        target = doc.simulation.newPath?.d1_category || '미분류';
      }
      const key = `${source}|||${target}`;
      flowMap[key] = (flowMap[key] || 0) + 1;
    });
